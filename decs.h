...
namespace decs
{
	...
	{
		privatised_statics::deltaTime = dt;
	}

	inline float World::getDeltaTime()
	{
		return privatised_statics::deltaTime;
	}

	inline int World::createNewID()
	{
		if (privatised_statics::reusableIds.empty())
		{
			return privatised_statics::nextAvailableID++;
		}
		int returnedID = privatised_statics::reusableIds.back();
		privatised_statics::reusableIds.pop_back();
		return returnedID;
	}

	inline bool World::addSystem(SystemBase& system)
	{
		size_t size = privatised_statics::systems.size();

		for (int i = 0; i < size; i++)
		{
			if (system.getSystemID() == privatised_statics::systems.at(i).get().getSystemID())
			{
				return false;
			}
		}
		privatised_statics::systems.emplace_back(system);
		return true;
	}

	inline int World::createNewSystemID()
	{
		return privatised_statics::assignableSystemID++;
	}
	...
	inline void World::update()
	{
		...
		for (int i = 0; i < size; i++)
		{
			privatised_statics::systems.at(i).get().update();
		}
	...
	inline void World::destroyEntity(int entityID, bool poolComponents)
	{
		...
		for (int i = 0; i < size; i++)
		{
			if (privatised_statics::systems.at(i).get().hasComponentWithID(entityID))
			{
				if (poolComponents)
				{
					privatised_statics::destroyListPool.push_back(entityID);
					break;
				}
				privatised_statics::destroyList.push_back(entityID);
				break;
			}
		}
	}
	...
	{
		size_t systemSize = privatised_statics::systems.size();
		int highestID = 0;

		for (int i = 0; i < systemSize; i++)
		{
			int highestSystemID = privatised_statics::systems.at(i).get().highestIDUsed();
	...
	inline void World::destroyOrphanedEntities()
	{
		...
		for (int i = 0; i <= highestID; i++)
		{
			...
			{
				if (!privatised_statics::systems.at(systemID).get().hasComponentWithID(i))
				{
		...
		{
			privatised_statics::reusableIds.push_back(destroyList.back());
	...
	{
		while (privatised_statics::destroyList.empty() == false)
		{
			for (int i = 0; i < privatised_statics::systems.size(); i++)
			{
				privatised_statics::systems.at(i).get().destroyAllComponentsWithID(privatised_statics::destroyList.back());
			}
			privatised_statics::reusableIds.push_back(privatised_statics::destroyList.back());
			privatised_statics::destroyList.pop_back();
		}
		while (privatised_statics::destroyListPool.empty() == false)
		{
			for (int i = 0; i < privatised_statics::systems.size(); i++)
			{
				privatised_statics::systems.at(i).get().removeAllComponentsWithID(privatised_statics::destroyListPool.back());
			}
			privatised_statics::reusableIds.push_back(privatised_statics::destroyListPool.back());
			privatised_statics::destroyListPool.pop_back();
		}
	} // end Destroy();
